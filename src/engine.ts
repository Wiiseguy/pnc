...
import { GameInfo, CustomContext, GameRoom, GameAction, GameActor, GameSound, BoundingBox } from './engine.core.ts'
...
globalThis.SOUND = function (name, fileName, options) {
    if (!fileName.startsWith('http')) {
        fileName = globalThis.ASSET(fileName)
    }
    options = {
        volume: 1,
        ...options
...
function _runActions() {
    ...
    console.log(
        'Running actions:',
        actionQueue.map(a => a.name + ' (' + a.duration + 'ms)')
    )
    _runNextAction(0)
}
function runActions() {
...
globalThis.PLAYSOUND = function (name, options) {
    addAction('PlaySound', () => {
        ...
        if (!sound?.sound) {
            console.error(`Sound ${name} not found or not loaded`)
            return
        }
        options = {
...
function initialize() {
    ...
    if (!gameInfo.skipIntro) {
        ...
        if (gameInfo.authors.length != 0)
            P5.text('by ' + gameInfo.authors.join(' & '), 0, gameInfo.fontSize, gameInfo.width, gameInfo.height)
        P5.textSize(gameInfo.fontSize)
    }
    ...
    P5.draw = _ => {
        if (!currentRoom) throw new Error('No current room set')
        if (renderMode === '3d') {
        ...
        ;[...currentRoom.actors, ...currentRoom.hotspots].forEach(h => {
            if (!h.visible) return
            if (
                h.actions.some(a => a.name === currentVerb || a.name === '*') &&
                h.boundingBox.contains(P5.mouseX, P5.mouseY)
            ) {
                hovering = true
                hoveringObject = h
            }
        ...
        if (isActionRunning) {
            ...
            P5.cursor(ASSET('/data/TestAdv/cur_pointer.png'))
        }
        ...
        if (currentText) {
            ...
            P5.text(
                currentText,
                gameInfo.fontSize,
                gameInfo.height - gameInfo.fontSize * 4,
                gameInfo.width - gameInfo.fontSize * 2,
                gameInfo.fontSize * 4,
                'center',
                'bottom'
            )
        ...
        if (isDebug) {
            ...
            actionQueue.forEach((action, i) => {
                P5.text(
                    `${action.name} (${action.duration ?? 0}ms)`,
                    10,
                    30 + gameInfo.fontSize / 2 + i * gameInfo.fontSize
                )
            })
            P5.pop()
        }
...
const P5 = new p5(sketch => {
    ...
    // The p5 instance is created here
    sketch.preload = function () {
        console.log('Preload')

        if (renderMode === '3d') {
            font = P5.loadFont(ASSET('/data/Swansea.ttf'))
        }
        ...
        gameInfo.images.forEach(img => {
            ...
            img.image = P5.loadImage(
                img.fileName,
                image => {
                    console.log('Loaded image:', img.name, img.fileName, image)
                    img.sx = img.sx ?? 0
                    img.sy = img.sy ?? 0
                ...
                failEvent => {
                    console.error('Failed to load image:', img.name, img.fileName, failEvent)
                }
            )
        })
        ...
        gameInfo.sounds.forEach(sound => {
            sound.sound = P5.loadSound(sound.fileName, () => {
                console.log('Loaded sound:', sound.name, sound.fileName, sound.sound)
                //sound.sound.play()
            })
        })
